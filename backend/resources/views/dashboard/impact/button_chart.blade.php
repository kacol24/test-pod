        ...
        <div class="col-md-8">
            <div class="py-3">
                <h1 class="Content__title Content__title--sm d-flex align-items-center mb-3">
                    <span class="Avatar border-0 mr-3" style="background-color:{{ $themeColor  }};">
                        <i class="fas fa-link fa-fw"></i>
                    </span>
                    Number of Leads (contact)
                </h1>
                <div id="impact_buttons_chart"></div>
            </div>
        </div>
                ...
                <div class="my-5 w-100">
                    ...
                    @if($visitors['sum'])
                        <small class="text-muted">
                            dari
                            {{ $visitors['sum_formatted'] }}
                            pengunjung
                            ({{ round($chart['sum'] / $visitors['sum'] * 100) }}%)
                        </small>
                    @endif
                </div>
...
<script>
    Highcharts.chart('impact_buttons_chart', {
        title: false,
        colors: ['{{ $themeColor }}'],
