...
@section('content')
    <main class="pb-3 pb-md-5" role="main" style="">
        ...
        @endforeach
        <form class="form-validate" action="{{route('optionset.store')}}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
                        ...
                        <div class="row no-gutters justify-content-between mb-3">
                            <div class="col-md-auto">
                                <h1 class="page-title m-0">
                                    {{Lang::get('option.addproductoptionset')}}
                                </h1>
                            </div>
                        ...
                        <div class="card p-0 mt-3">
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="title">{{Lang::get('option.optionsetname')}}</label>
                                    <input type="text" class="form-control" name="title" id="title"
                                           placeholder="ex: Size & Color" value="{{old('title')}}" required="true">
                                </div>
                                ...
                                <div class="row">
                                    @foreach($options as $option)
                                        <div class="col-6">
                                            <label>
                                                <input type="checkbox" value="{{$option->id}}" name="selected[]">
                                                {{$option->title}}
                                            </label>
                                        </div>
                                    @endforeach
